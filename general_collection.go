...
type GeneralCollection[T any] struct {
	...
	autoGeneratedKeys map[string]bool // 用于标记哪些item的key是自动生成的，这些key不会在覆盖到新产生的集合中(例如 FilterBy, GroupBy 这类方法所生成的新集合)
	keysIndex         map[string]int  // 用于通过key搜索数据的次序下标
	sliceCache        []T
	defaultItem       T
}

// NewGeneralCollection create a GeneralCollection, init with items
func NewGeneralCollection[T any](items ...T) *GeneralCollection[T] {
	c := &GeneralCollection[T]{
		items:             make(map[string]T),
		orderedKeys:       make([]string, 0),
		autoGeneratedKeys: make(map[string]bool),
	}
...
// Add append an item into the collection
func (c *GeneralCollection[T]) Add(items ...T) {
...
// AddWithKey try to append an item with its search key into the collection.
// If the key already exists, it will override the item, but keep the item order in the collection.
func (c *GeneralCollection[T]) AddWithKey(item T, key string) {
...
func (c *GeneralCollection[T]) Merge(collections ...*GeneralCollection[T]) {
...
func (c *GeneralCollection[T]) MergeSlices(slices ...[]T) {
...
func (c *GeneralCollection[T]) MergeMaps(keepKeys bool, maps ...map[string]T) {
...
// GroupCountBy 根据给定的分组逻辑，计算出每个分组中数据项的数量, 拆分后的组的键由参数中的分组逻辑提供
func (c *GeneralCollection[T]) GroupCountBy(grouper func(item T, idx int, key string) string) map[string]int {
...
// GroupBy 根据给定的分组逻辑, 将这个集合拆分成多个集合组, 拆分后的集合组的键由参数中的分组逻辑提供
func (c *GeneralCollection[T]) GroupBy(grouper func(item T, idx int, key string) string) map[string]*GeneralCollection[T] {
	grouped := map[string]*GeneralCollection[T]{}
	for idx, key := range c.orderedKeys {
		item := c.items[key]

		groupKey := grouper(item, idx, key)
		if _, ok := grouped[groupKey]; !ok {
			grouped[groupKey] = NewGeneralCollection[T]()
		}
		if c.isAutoGenKey(key) {
			grouped[groupKey].Add(item)
		} else {
			grouped[groupKey].AddWithKey(item, key)
		}
...
func (c *GeneralCollection[T]) SelfFilterBy(matcher func(item T, idx int, key string) bool) {
	for idx, key := range c.orderedKeys {
		item := c.items[key]
		if !matcher(item, idx, key) {
			c.RemoveByIndex(idx)
